...
> Fuente principal: `docs/01-bitacora/requisitos/normalizados/`.
> Fuentes oficiales originales: `docs/00-material/`.
> Rúbrica documental: `docs/09-entrega-final/referencias-evaluacion/rubrica.md`.
...
| --------------------------- | --------------------------------------------- | --------------------------- | ------------------------------------------------------------ | -------------------------- |
| Definición funcional previa | `docs/01-bitacora/requisitos/`                         | Markdown legacy             | Comparar solape con normalización y rescatar contenido único | Parcial                    |
| Requisitos normalizados     | `docs/01-bitacora/requisitos/normalizados/`               | Markdown normalizado        | Fuente principal para alcance, trazabilidad y sprints        | Revisado                   |
| PDFs originales             | `docs/00-material/`                           | PDF oficial                 | Fuente oficial de contraste; no tocar                        | Revisado                   |
| Rúbrica documental          | `docs/09-entrega-final/referencias-evaluacion/rubrica.md`                             | Markdown                    | Criterio real de evaluación documental                       | Revisado                   |
...
| Multimedia local            | `img/`, `docs/01-bitacora/diw/media/`                  | Imágenes y vídeo local      | Verificar recursos para vídeo/anuncio y Sprint 3 DIW         | Parcial                    |
...
| ------------------------------- | ------ | -------------------------------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `docs/01-bitacora/requisitos/`           | Sí     | Definición funcional temprana del producto         | `01-definicion-proyecto.md` con concepto, reglas de negocio y módulos | Solape parcial con `01_resumen_global_proyecto.md` y requisitos por sprint | Mantener temporalmente como fuente complementaria, no como carpeta principal de requisitos normalizados |
| `docs/01-bitacora/requisitos/normalizados/` | Sí     | Normalización estructurada de requisitos oficiales | Índice, resumen global, rúbrica, módulos y sprints 1/2/3              | Solapa con la definición inicial, pero es más completa y sistemática       | Mantener como fuente principal para el equipo y la auditoría técnica                                    |
...
- La carpeta principal que debe mantenerse como fuente normalizada es `docs/01-bitacora/requisitos/normalizados/`.
- `docs/01-bitacora/requisitos/` no debe seguir creciendo como estructura paralela de requisitos normalizados.
- El archivo `docs/01-bitacora/requisitos/01-definicion-proyecto.md` contiene contenido útil y único sobre concepto, reglas de negocio y alcance funcional temprano; no debe perderse.
...
- Acción recomendada para fase posterior, sin ejecutarla ahora: fusionar el contenido único de `docs/01-bitacora/requisitos/01-definicion-proyecto.md` dentro de la documentación principal o reclasificarlo explícitamente como documento conceptual histórico, dejando `docs/01-bitacora/requisitos/normalizados/` como única referencia normalizada.
...
| Despliegue                             | pendiente                        | `docs/01-bitacora/despliegue/sprint-3/` está vacío y `docs/evidencias/despliegue/sprint-3/` no contiene evidencia local.                                                                                   |
...
| Recursos multimedia para vídeo/anuncio | documentado pero no implementado | Existen `img/landing/pacepalTienda.mp4`, `docs/01-bitacora/diw/media/pacepal_video_base_20s.mp4` y piezas de concurso, pero no hay integración visible en vistas ni trazabilidad clara al Sprint 3. |
...
| Requisitos y trazabilidad Sprint 1/2/3                  |  10% |            70% |      7,00 | Existen los 10 archivos normalizados y documentación por sprint; sigue habiendo solape con `docs/01-bitacora/requisitos/` y falta trazabilidad directa requisito-código-evidencia. |
...
| DWEC: buscador dinámico / AJAX                          |  10% |            35% |      3,50 | La propia documentación (`docs/01-bitacora/dwec/sprint-2/02-buscador-ajax.md`) reconoce un filtro local en memoria, sin búsqueda remota ni paginación.                             |
...
| Despliegue / publicación                                |   5% |            15% |      0,75 | Hay carpetas previstas, pero `docs/01-bitacora/despliegue/sprint-3/` y `docs/evidencias/despliegue/sprint-3/` siguen vacías; publicación remota no se audita aquí.                        |
...
| ------------------------------------------------------------ | ------------------ | ------------------------------------------------------------------------- |
| `docs/01-bitacora/requisitos/normalizados/04_dwec_cliente_requisitos.md` | revisar            | Confirmar el alcance exacto de Sprint 3 antes de ejecutar trabajo técnico |
...
**Problema detectado:**  
Hay recursos multimedia locales (`img/landing/pacepalTienda.mp4`, `docs/01-bitacora/diw/media/pacepal_video_base_20s.mp4`), pero no aparecen referenciados en páginas, JS o documentación operativa de Sprint 3. Tampoco hay evidencia local de audio integrado y solo se observan animaciones menores de transición/cookies.
...
| Archivo/ruta                                   | Acción  | Motivo                                                  |
| ---------------------------------------------- | ------- | ------------------------------------------------------- |
| `docs/01-bitacora/diw/README.md`                        | editar  | Debe reflejar Sprint 3 y no quedarse en Sprint 1/2      |
| `docs/01-bitacora/diw/media/pacepal_video_base_20s.mp4` | revisar | Recurso local existente pendiente de trazabilidad       |
...
**Problema detectado:**  
`docs/01-bitacora/despliegue/sprint-3/` está vacío y `docs/evidencias/despliegue/sprint-3/` también. El requisito de despliegue local en XAMPP no tiene documentación ni evidencia local verificable.
...
| Archivo/ruta                           | Acción                           | Motivo                                                            |
| -------------------------------------- | -------------------------------- | ----------------------------------------------------------------- |
| `docs/01-bitacora/despliegue/sprint-3/`                | crear archivos dentro            | La carpeta existe, pero no contiene documentación                 |
...
**Problema detectado:**  
Hay desalineaciones graves entre documentación y código real. El README raíz tiene ruta antigua y árbol desfasado. `docs/01-bitacora/dwes/sprint-2/endpoints-api.md` no coincide con el router real. `docs/01-bitacora/dwes/sprint-2/modelo-relacional.md` no coincide del todo con `db/schema.sql`. `docs/evidencias/dwes/README.md` niega evidencias que sí existen. La rúbrica ya está incorporada a `docs/01-bitacora/requisitos/normalizados/02_rubrica_documentacion.md`, pero falta propagar esa precisión a la documentación operativa del proyecto.
...
| `docs/01-bitacora/dwes/sprint-2/endpoints-api.md`                   | editar  | Documenta endpoints que no coinciden con `src/api/index.php`               |
| `docs/01-bitacora/dwes/sprint-2/modelo-relacional.md`               | editar  | Resume campos/relaciones que no coinciden plenamente con `db/schema.sql`   |
| `docs/evidencias/dwes/README.md`                           | editar  | Dice que no hay evidencias, pero sí hay imágenes locales                   |
| `docs/01-bitacora/requisitos/normalizados/02_rubrica_documentacion.md` | revisar | Ya incorpora `docs/09-entrega-final/referencias-evaluacion/rubrica.md`; debe mantenerse como referencia central    |
...
**Problema detectado:**  
`docs/01-bitacora/requisitos/01-definicion-proyecto.md` y `docs/01-bitacora/requisitos/normalizados/` cumplen funciones cercanas pero no idénticas. Si no se resuelve la duplicidad en una fase posterior, el equipo o los mantenedores del proyecto pueden trabajar sobre dos fuentes de verdad.
...
| Archivo/ruta                                                 | Acción  | Motivo                                                      |
| ------------------------------------------------------------ | ------- | ----------------------------------------------------------- |
| `docs/01-bitacora/requisitos/01-definicion-proyecto.md`               | revisar | Contiene alcance funcional y reglas de negocio tempranas    |
| `docs/01-bitacora/requisitos/normalizados/01_resumen_global_proyecto.md` | revisar | Documento global que ya cubre buena parte del marco oficial |
...
| `docs/01-bitacora/dwec/sprint-2/08-tests-funcionales-sprint-2.md` | editar  | Hoy reconoce validación manual genérica                                          |
...
| `docs/01-bitacora/diw/media/pacepal_video_base_20s.mp4` | revisar | Posible base audiovisual del proyecto         |
| `img/landing/pacepalTienda.mp4`                | revisar | Vídeo local de tienda/landing                 |
| `docs/01-bitacora/sostenibilidad/concurso/`             | revisar | Material visual complementario                |
...
| Archivo/ruta                            | Acción  | Motivo                                            |
| --------------------------------------- | ------- | ------------------------------------------------- |
| `docs/01-bitacora/dwes/sprint-2/roles-acceso.md` | editar  | Describe capacidades admin que deben contrastarse |
...
| --------------------------------------------------------------- | ---------- | --------- | -------------------------------------------------- |
| `docs/01-bitacora/despliegue/sprint-3/01-despliegue-local-xampp.md`             | despliegue | P0        | Guía reproducible del entorno local                |
| `docs/01-bitacora/despliegue/sprint-3/02-validacion-localhost-bbdd.md`          | despliegue | P0        | Validación técnica del entorno local               |
...
| `docs/01-bitacora/dwes/sprint-2/endpoints-api.md`                 | DWES          | P0        | Alinear endpoints y métodos con `src/api/index.php`                                         |
| `docs/01-bitacora/dwes/sprint-2/modelo-relacional.md`             | DWES          | P0        | Alinear modelo documental con `db/schema.sql` y evidencias reales                           |
...
| `docs/01-bitacora/diw/README.md`                                  | DIW           | P0        | Incluir Sprint 3 multimedia y trazabilidad de recursos                                      |
| `docs/01-bitacora/dwec/sprint-2/02-buscador-ajax.md`              | DWEC          | P0        | Ajustar el documento al comportamiento real o convertir la brecha en tarea explícita        |
| `docs/01-bitacora/dwec/sprint-2/08-tests-funcionales-sprint-2.md` | tests         | P1        | Sustituir validación genérica por casos concretos y resultados                              |
| `docs/01-bitacora/dwes/sprint-2/roles-acceso.md`                  | DWES          | P1        | Ajustar permisos y alcance admin a la implementación real                                   |
...
| `docs/01-bitacora/diw/media/pacepal_video_base_20s.mp4`  | multimedia | P1        | Si es recurso base o evidencia final                 |
...
| ------------------------------------------------------------------------ | ------- | --------- | --------------------------------------------------------------------------------------------------------- |
| Requisitos mal interpretados                                             | Alto    | P0        | Trabajar siempre desde `docs/01-bitacora/requisitos/normalizados/` y contrastar con `docs/00-material/`               |
...
| Documentación no alineada con rúbrica                                    | Alto    | P0        | Usar `docs/09-entrega-final/referencias-evaluacion/rubrica.md` y `docs/01-bitacora/requisitos/normalizados/02_rubrica_documentacion.md` como base de revisión |
...
| Duplicidad entre `docs/01-bitacora/requisitos/` y `docs/01-bitacora/requisitos/normalizados/` | Medio   | P1        | Resolver en tarea controlada y posterior                                                                  |
| Equipo trabajando sobre dos fuentes de verdad distintas                  | Alto    | P1        | Declarar una sola fuente principal: `docs/01-bitacora/requisitos/normalizados/`                                       |
...
- [x] `docs/01-bitacora/requisitos/` revisado.
- [x] `docs/01-bitacora/requisitos/normalizados/` revisado.
...
| Sprint 0: base documental y backlog inicial | cerrada | `docs/01-bitacora/agile/03-notas-sprint0.md`, `docs/evidencias/01-sprint0/sprint0-tablero.png`, `docs/evidencias/01-sprint0/sprint0-arbol.png` | Existe trazabilidad local del arranque del proyecto                                                                                                                                         |
| Definición funcional inicial del proyecto   | cerrada | `docs/01-bitacora/requisitos/01-definicion-proyecto.md`                                                                                        | Documento funcional amplio, aunque ya no debe ser la fuente principal normalizada                                                                                                           |
| Wireframes base de la aplicación            | cerrada | `docs/01-bitacora/diw/sprint-1/03-wireframes.md`, `docs/01-bitacora/diw/wireframes/`                                                                    | Evidencia local clara del diseño inicial                                                                                                                                                    |
...
| Auditoría de accesibilidad de Sprint 2      | cerrada | `docs/01-bitacora/diw/sprint-2/01-accesibilidad-wcag.md`, `docs/evidencias/diw/sprint-2/`                                                      | Hay capturas locales y documento específico                                                                                                                                                 |
| Normalización de requisitos y rúbrica       | cerrada | `docs/01-bitacora/requisitos/normalizados/00_indice_requisitos.md` a `09_sprint_3_requisitos.md`                                                  | La base normalizada ya existe y la rúbrica real está incorporada                                                                                                                            |
...
- Recomendación final sobre carpetas de requisitos: mantener `docs/01-bitacora/requisitos/normalizados/` como fuente principal y tratar `docs/01-bitacora/requisitos/01-definicion-proyecto.md` como documento complementario o histórico hasta resolver su integración en una fase posterior.
