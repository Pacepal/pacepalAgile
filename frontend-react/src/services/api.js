...
function detectProjectApiBaseUrl() {
    ...
    if (pathname.includes('/frontend-react/dist/')) {
        return new URL('../../src/api/index.php/api', window.location.href).href;
    }

    if (pathname.includes('/frontend-react/')) {
        return new URL('../src/api/index.php/api', window.location.href).href;
    }
...
function buildApiWarning(baseUrl) {
    ...
    if (isViteLocalRuntime()) {
        return 'React esta ejecutandose desde Vite, pero falta VITE_PACEPAL_API_BASE_URL en frontend-react/.env.local.';
    }
